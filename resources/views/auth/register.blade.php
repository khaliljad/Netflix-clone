    ...
    <div class="wrapper-login">
        ...
        <div class="login register">
          <form class="signin-form"  method="POST" action="{{ route('register') }}">
            ...
            <h1 class="title">{{__('login.Sign Up')}}</h1>
  
            <div class="field">
              <input id="firstName" type="text" class="text-input @error('firstName') is-invalid @enderror" name="firstName" value="{{ old('firstName') }}" required  autofocus />
              <span class="floating-label">{{__('login.First Name')}}</span>
            ...
            <div class="field">
              <input id="nalastNameme" type="text" class="text-input @error('lastName') is-invalid @enderror" name="lastName" value="{{ old('lastName') }}" required />
              <span class="floating-label">{{__('login.Last Name')}}</span>
            ...
            <div class="field">
              <input type="text" class="text-input @error('age') is-invalid @enderror" name="age" value="{{ old('age') }}" required />
              <span class="floating-label">{{__('login.Age')}}</span>
            ...
            <div class="field">
              <input id="email" type="email" class="text-input @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email" />
              <span class="floating-label">{{__('login.Email address')}}</span>
            ...
            <div class="field">
                <input id="mobile" type="mobile" class="text-input @error('mobile') is-invalid @enderror" name="mobile" value="{{ old('mobile') }}" required>
                <span class="floating-label">{{__('login.Mobile')}}</span>
                    @error('mobile')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            <div class="field">
              <input id="password" type="password" class="text-input @error('password') is-invalid @enderror" name="password" required autocomplete="new-password" />
            ...
            <div class="field">
              <input class="text-input" id="password-confirm" type="password" name="password_confirmation" required autocomplete="new-password" />
              <span class="floating-label test">{{__('login.Confirm Password')}}</span>
            </div>
  
            <button type="submit" class="signin-btn">{{__('login.Sign Up')}}</button>
            ...
            <div class="face-login">
                ...
                <a href="redirect/facebook"><span class="login-with-fb">{{__('login.Sign Up with Facebook')}}</span></a>
            </div>
  
            <div class="onboarding">
              
              <p>{{__('login.Already have an account?')}} <a href="{{ route('login') }}">{{__('login.Sign in now.')}}</a></p>
            </div>
          </form>
        </div>
